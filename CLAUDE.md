...
- Admin mutations follow [actions/skills.ts](src/app/admin/actions/skills.ts): `requireAdmin()` → Zod schema from `validations.ts` → `src/models` → `revalidatePath("/")` + `revalidatePath("/admin", "layout")`. They return `ActionState` ([src/lib/actionState.ts](src/lib/actionState.ts)) for `useFormState`. Admin forms take their classes from [src/components/admin/styles.ts](src/components/admin/styles.ts) and use its `SubmitButton`.
