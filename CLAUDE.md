...
- Admin mutations follow [actions/skills.ts](src/app/admin/actions/skills.ts): `requireAdmin()` → Zod schema from `validations.ts` → `src/models` → `revalidatePath("/")` + `revalidatePath("/admin", "layout")`. They return `ActionState` ([src/lib/actionState.ts](src/lib/actionState.ts)) for `useFormState`, with errors keyed by dotted path (`projects.0.name`). Admin forms build on [src/components/admin/](src/components/admin/): `styles.ts` (class strings, kept out of `"use client"` so server components get real strings), `Field` / `fieldProps` / `Checkbox`, and `SubmitButton`. Repeatable rows (experience metrics/projects) are client state posted as JSON hidden inputs and parsed by the schema.
