      ...
      <div className="grid grid-cols-1 md2:grid-cols-2 gap-px bg-border border border-border rounded-xl overflow-hidden">
        {projects.map((p, i) => (
          <ScrollReveal key={p.id} delay={i * 0.05} className="relative bg-white p-8 cursor-none group hover:bg-black transition-colors duration-[220ms] overflow-hidden">
            <div className="absolute bottom-6 right-6 text-[5rem] font-black text-gray-2 leading-none font-mono pointer-events-none group-hover:text-white/8 transition-colors duration-[220ms] z-0">{p.id}</div>
            <div className="relative z-10">
              ...
              <div className="flex flex-wrap gap-[0.3rem]">
                {p.tags.map((t) => <span key={t} className="text-[0.62rem] font-semibold px-2 py-0.5 rounded-[3px] bg-gray text-text2 border border-border group-hover:bg-white/8 group-hover:text-white/60 group-hover:border-white/10 transition-all duration-[220ms]">{t}</span>)}
              </div>
            </div>
          </ScrollReveal>
        ))}
